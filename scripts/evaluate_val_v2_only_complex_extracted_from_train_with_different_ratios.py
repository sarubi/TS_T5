...
datasets_to_evaluate = {
    "gold_ratios" : "processed_data/wikilarge.val_v2_only_complex_extracted_from_train/gold_ratios",
    "lr_ratios" : "processed_data/wikilarge.val_v2_only_complex_extracted_from_train/0_linear_regression_models",
    # "roberta_ratios" : "processed_data/wikilarge.val_v2_only_complex_extracted_from_train/roberta_lr",
    "fixed_ratios" : "processed_data/wikilarge.val_v2_only_complex_extracted_from_train/0_fr_ratios"
}

for dataset_name_key, filepath in datasets_to_evaluate.items():
    output_dir = dataset_base_dir_name / f'{dataset_name_key}-filtered_wiki.complex_only.valid_v2-t5ft-{experiment_name}/maxdepdepth_-1_maxdeplength_-1_diffwordscount_-1_avgwordcount_-1_length_-1_leven_-1'
    output_dir.mkdir(parents=True, exist_ok=True)
    print(f"Evaluate dataset: {filepath}, output will be saved to: {output_dir}")

    # Define the source file paths
    complex_source_filepath = dataset_base_dir_name / f'{filepath}/filtered_wiki.complex_only.valid_v2.src'
    simple_target_filepath = dataset_base_dir_name / f'{filepath}/filtered_wiki.complex_only.valid_v2.tgt'
    complex_processed_filepath = dataset_base_dir_name / f'{filepath}/filtered_wiki.complex_only.valid_v2.src_processed'
